...
from backend.app.strategy_skill import (
    DEFAULT_SKILL,
    normalize_skill,
...
def test_skill_is_human_readable_and_preserves_invariants():
    skill = normalize_skill(
        {
            "name": "测试 Skill",
            "lessons": [
                {"trigger": "出现单一例子", "action": "回到总体比较", "confidence": 0.8}
            ],
        },
        skill_id="skill-test",
        parent="baseline-v1",
    ...
    markdown = render_skill_markdown(
        skill, {"status": "candidate", "games": 8, "topics": 2, "win_rate": 0.625}
    )
...
def test_evolution_updates_skill_across_topics_and_iterations(monkeypatch):
    ...
    workspace = {
        ...
        "analysis": {
            ...
            "conflicts": ["工具增益与能力依赖"],
            "criterion": "长期净影响",
            "burdens": {"正方": "证明提升", "反方": "证明未提升"},
        },
        "arguments": [],
        "evidence": [],
        "matrix": [],
        "insights": [],
        "drafts": {"立论": "初稿", "攻辩": [], "自由辩论": [], "总结": "总结"},
        "warnings": [],
    }
    ...
    async def fake_judge(topic, transcript):
        candidate_turn = next(
            ...
            if turn["meta"].get("strategy_id", "").startswith("skill_")
        )
        return {
            "winner": candidate_turn["stance"],
            "scores": {},
            "missed_responses": ["需要更快回扣原题"],
            "highlights": [],
        }
    ...
    with client:
        project = client.post(
            "/api/projects", json={"topic": workspace["topic"], "stance": "正方"}
        ).json()
        ...
        stats = client.get("/api/evolution/trajectories/stats").json()
        assert stats == {"total": 8, "topics": 2, "outcomes": {"success": 8}}
        trajectory = client.get(
            f"/api/evolution/trajectories/{result['game_results'][0]['trajectory_id']}"
        ).json()
        assert len(trajectory["transcript"]) == 4
        ...
        assert reused["aggregate_games"] == 0
        assert reused["reused_trajectories"] == 8
        assert reused["estimated_logical_llm_calls"] == 1
        assert reused["promoted"] is False
        ...
        assert incremental["aggregate_games"] == 1
        assert incremental["reused_trajectories"] == 8
        assert incremental["estimated_logical_llm_calls"] == 7
        assert incremental["skill"]["version"] == "1.2.0"
        assert incremental["promoted"] is False
        assert client.get("/api/evolution/trajectories/stats").json()["total"] == 9
