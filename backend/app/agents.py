...
def naturalize_challenge_language(speech: str) -> str:
    """Rewrite host-like or pleading openers into an adversarial register.

    Deleting the soft opener is not enough: "请问对方能不能给出一个例子" is
    still a classroom request once the "请问" is gone. Each rule rewrites the
    request itself, so the challenge reads like one.
    """
    text = speech
    replacements = (
        (r"[，,、]?请(?:你|对方(?:辩友)?)?(?:给出|提供|举出|举|拿出)(?:一个|一些|几个|个)?", "试问对方，能拿出什么"),
        (r"[，,、]?请(?:你|对方(?:辩友)?)?(?:正面)?回答(?:一下)?[：:，,、\s]*", "试问对方，"),
        (r"请(?:你|对方(?:辩友)?)?比较(?:一下)?", "真正需要比较的是"),
        (r"请(?:你|对方(?:辩友)?)?说明(?:一下)?", "问题在于"),
        (r"请(?:你|对方(?:辩友)?)?解释(?:一下)?", "问题在于"),
        (r"请(?:你|对方(?:辩友)?)?(?:证明|举证)(?:一下)?[：:，,、\s]*", "对方至今没有证明"),
        (r"[，,、]?请问(?:对方(?:辩友)?)?[：:，,、\s]*", "试问对方，"),
    )
    for pattern, replacement in replacements:
        text = re.sub(pattern, replacement, text)
    # A yes/no permission request is the softest way to ask for specifics.
    # Only rewrite clauses that actually end in a question, so an embedded
    # "这取决于对方能否给出证据。" is left alone. Runs after the openers above.
    text = re.sub(
        r"(?:对方(?:辩友)?)?(?:能否|能不能|是否可以)(?:给出|提供|举出|举|拿出)(?:一个|一些|几个|个)?([^。！？]*？)",
        lambda match: f"试问对方，能拿出什么{match.group(1)}",
        text,
    )
    # Stacked rules can emit the marker twice.
    text = re.sub(r"(?:试问对方，\s*){2,}", "试问对方，", text)
...
先准确、简短地回应对方最新发言里最影响结论的一个质疑，再给本方理由。交锋语气必须是对抗性的，不是求教的：禁止使用“请回答”“请比较”“请说明”“请证明”“请问对方”等主持人式模板，也禁止“能否”“是否可以”“对方可不可以”“请您给出”等征询式说法。质疑要用反问句或直接判断正面施压，例如“试问对方，……”“难道……”“对方凭什么说……”“对方至今没有解释……”“给不出这个条件，对方的结论就不成立”。不要写“请问对方能不能给出一个具体例子”，要写“试问对方，能拿出哪个具体例子”。自由辩论也不要求每轮都以问句结尾：推理已经把对方堵死时，直接用一句明确判断收束。不要同时铺开多个次要攻击点，也不要重述已经说清的背景。对方没有发言时直接提出本方观点，不要虚构对方观点。不强行给对方扣逻辑谬误的帽子，也不要只要求对方举证而不给自己的判断。
...
发言以{min_chars}-{max_chars}个中文字符为目标，且不得超过{hard_max_chars}字。通常用3-4个自然完整的句子：直接回答、一条因果理由，再用一个反问或一句明确判断收束。不得为压缩字数省略必要主谓宾或把多层逻辑硬塞进一句。允许少于下限，前提是回应、理由和收束已经说清。
...
待废弃初稿：{json.dumps(result, ensure_ascii=False)}"""
        ...
        target = user_text[:60]
        return {
            "speech": f"对方刚才强调“{target}”，但该子问题只有在能改变原题《{topic}》的结论时才重要。这里缺少了从现象到结论的关键一步：即便该现象存在，也不等于它足以决定本题。试问对方，凭什么说这一影响足以压过其他成本？给不出这个比较，“{criterion}”这条基线就没有被动摇，对方也仍未完成对原题的举证。",
            "target": target,
            "tactic": "追问并争夺标准",
            "issue": "因果链或比较基线缺失",
